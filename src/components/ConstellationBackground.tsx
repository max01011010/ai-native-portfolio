...
const ConstellationBackground: React.FC<ConstellationBackgroundProps> = ({
  ...
  lineColor = "rgba(255, 255, 255, ", // Opacity will be added dynamically
    ...
    for (let i = 0; i < particles.current.length; i++) {
      for (let j = i + 1; j < particles.current.length; j++) {
        ...
        if (distance < MAX_LINE_DISTANCE) {
          ...
          ctx.strokeStyle = `${lineColor}${1 - distance / MAX_LINE_DISTANCE})`; // Fade lines based on distance
          ctx.lineWidth = 0.5;
          ctx.stroke();
        }
      }
  ...
  return (
    <div className={cn("absolute inset-0 z-0", className)}>
      ...
      <div className={cn("absolute inset-0", overlayColor)} style={{ opacity: overlayOpacity }}></div>
    </div>
  );
};
